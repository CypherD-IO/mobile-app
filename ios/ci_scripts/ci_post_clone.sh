...
# Write CioSecrets.xcconfig from Xcode Cloud environment variable.
# The Podfile post_install hook injects an #include for this file into the
# NotificationServiceExtension's Pods xcconfig, so it MUST exist before pod install.
CIO_SECRETS_PATH="${REPO_ROOT}/ios/CioSecrets.xcconfig"
if [ -z "${CUSTOMERIO_CDP_API_KEY:-}" ]; then
  echo "WARNING: CUSTOMERIO_CDP_API_KEY is not set — CioSecrets.xcconfig will be empty."
  echo "// CUSTOMERIO_CDP_API_KEY was not provided at build time." > "$CIO_SECRETS_PATH"
else
  cat > "$CIO_SECRETS_PATH" << EOL
// Auto-generated by ci_post_clone.sh — DO NOT commit this file.
CUSTOMERIO_CDP_API_KEY = ${CUSTOMERIO_CDP_API_KEY}
EOL
  chmod 600 "$CIO_SECRETS_PATH"
  echo "Successfully created CioSecrets.xcconfig at: $CIO_SECRETS_PATH"
fi
