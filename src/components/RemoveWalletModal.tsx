  ...
  const deleteTheWallet = async () => {
    ...
    if (!isReadOnlyWallet) {
      ...
      if (data) {
        ...
        axios
          .delete(
            `${ARCH_HOST}/v1/configuration/address/${ethereum.address}/observer/${readOnlyWalletData.observerId}`,
          )
          .catch(error => {
            Sentry.captureException(error);
          });
    ...
    <CyDModalLayout
      ...
      <CyDView
        ...
        <CyDView>
          ...
          {importNewWallet && (
            <DynamicTouchView
              ...
              <DynamicTouchView
                ...
                width={18}
                mH={5}
                bGC={selectAcc ? 'black' : 'transparent'}
                onPress={() => {
                  setSelectAcc(!selectAcc);
                }}>
                {selectAcc && (
          ...
          {seedPharse && (
            <ButtonWithOutImage
              sentry-label='remove-wallet-seed'
              mT={20}
              wT={95}
              bG={Colors.appColor}
              vC={Colors.appColor}
          ...
          {importNewWallet && (
            <ButtonWithOutImage
              sentry-label='remove-wallet-remove'
              disable={!selectAcc}
              mT={10}
              bG={Colors.appColor}
              vC={Colors.appColor}
          ...
          {deleteWallet && (
            <ButtonWithOutImage
              sentry-label='delete-wallet'
              disable={!selectAcc}
              bG={selectAcc ? Colors.redColor : Colors.redOffColor}
              vC={Colors.redColor}
              mT={10}
