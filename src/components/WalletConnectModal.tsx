...
export default function WalletConnectModal (props) {
  ...
  const checkGasFee = () => {
    const item = find(ALL_CHAINS, { chainIdNumber: Number(params?.payload?.params[0]?.chainId) });
    if (item === undefined) {
  ...
  useEffect(() => {
    if (!walletConnectModalVisible && isChainSwitched) {
      if (!(method === EIP155_SIGNING_METHODS.ETH_SIGN || method === EIP155_SIGNING_METHODS.PERSONAL_SIGN || method === EIP155_SIGNING_METHODS.WALLET_GET_PERMISSIONS || method === EIP155_SIGNING_METHODS.WALLET_REQUEST_PERMISSIONS || method.includes('TypedData'))) {
        checkGasFee();
      }
    }
    ...
    if (method === EIP155_SIGNING_METHODS.PERSONAL_SIGN) {
      ...
      return (
        <CyDView className={'mx-[47px]'}>
          <CyDText className={'text-[17px] font-bold'}>{t<string>('MESSAGE')} :</CyDText>
          <CyDScrollView className={'max-h-[180px] bg-backLight p-[8px] mt-[10px] pb-[18px] rounded-[6px]'}>
            <CyDText className={'mt-[10px] text-[14px]'}>{message ?? ''}</CyDText>
          </CyDScrollView>
        </CyDView>
      );
  ...
  const renderButtonMessage = () => {
    if (method?.includes('TypedData')) return t('REVIEW_PERMISSION').toUpperCase();
