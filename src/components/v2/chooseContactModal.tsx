...
import { CyDFlatList, CyDImage, CyDText, CyDTextInput, CyDTouchView, CyDView } from '../../styles/tailwindStyles';
...
export default function ChooseContactModal (props: ContactModalProps) {
  ...
  const { isChooseContactModalVisible, onSelectingContact, onCancel, noContactsAvailableMessage = t<string>('NO_CONTACTS_TEXT') } = props;
  ...
  const searchContacts = (contactName: string) => {
    if (contactName !== '') {
      const contactNames = fuse.search(contactName).map(contact => contact.item);
      const filteredContacts = Object.fromEntries(Object.entries(contactBook).filter(([key, value]) => contactNames.includes(key)));
  ...
  const ContactItem = ({ name, addresses, imageProfile }: ContactItemProps) => {
    ...
    const RenderIcon = (contact: ContactItemProps) => {
      ...
      return (
        <CyDView className={'flex flex-row'}>
          ...
          {((Object.keys(contact.addresses).length - numberOfIcons) > 0) && <CyDView className={'p-[5px] rounded-[30px] ml-[-7px]'}>
    ...
    return (
        <CyDTouchView disabled={Object.values(filteredContactBook).length === 0} onPress={() => { onSelectingContact({ name, addresses, imageProfile }); }} className={clsx('flex flex-row justify-between border-b-[1px] border-b-sepratorColor mx-[15px]', { 'opacity-25': Object.values(filteredContactBook).length === 0 })}>
          <CyDView className={clsx('flex flex-row justify-between items-center w-[100%] py-[12px]')}>
            <CyDView className='flex flex-row justify-between items-center'>
              <CyDImage source={AppImages[`ADDRESS_PROFILE_${imageProfile}`]} className='h-[30px] w-[30px]' resizeMode='contain'/>
              <CyDText className='ml-[10px] text-[16px] font-bold'>{formatName(name).join('\n')}</CyDText>
            </CyDView>
            <CyDView className='flex flex-row justify-between items-center'>
                <CyDView className='flex flex-row mr-[5px]'>
                  <RenderIcon name={name} imageProfile={imageProfile} addresses={addresses} />
                </CyDView>
            </CyDView>
          </CyDView>
      </CyDTouchView>
          ...
          <CyDView className={'mt-[20px] mb-[100px]'}>
            <CyDView className={'flex flex-row justify-between items-center self-center border-[1px] border-sepratorColor w-[353px] h-[60px] rounded-[30px] px-[20px]'}>
              <CyDTextInput
                ...
                autoCapitalize="none"
                autoCorrect={false}
                onChangeText={(text: string) => { setSearchText(text); searchContacts(text); }}
            ...
            {Object.keys(filteredContactBook).length
              ...
              renderItem={ ({ item, index }: { item: ContactItemProps, index: number}) => <ContactItem key={index} name={item.name} imageProfile={item.imageProfile} addresses={item.addresses} /> }
