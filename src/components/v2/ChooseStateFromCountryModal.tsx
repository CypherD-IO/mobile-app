...
const ChooseStateFromCountryModal = ({
  isModalVisible,
  setModalVisible,
  selectedCountry,
  ...
  useEffect(() => {
    if (stateFilterText === '') {
      ...
      const tempStates = states.filter(state =>
      ...
      <CyDKeyboardAvoidingView
        ...
        <CyDView className={'bg-white h-[70%] rounded-t-[24px]'}>
          <CyDView
            ...
            <CyDTextInput
              className={
                'border-[1px] border-inputBorderColor rounded-[8px] p-[10px] text-[14px] w-[80%] font-manrope text-primaryTextColor'
              }
                ...
                return (
                  <CyDTouchView
                    ...
                    className={clsx(
                      ...
                      {
                        'bg-paleBlue': state.name === selectedState.name,
                      },
                    )}
                    key={state.id}>
                    <CyDView
                      className={'flex flex-row items-center max-w-[85%]'}>
