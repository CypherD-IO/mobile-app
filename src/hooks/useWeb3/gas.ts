...
export const calculateFeeSignAmino = (chainId: string, signDoc: StdSignDoc) => {
  ...
  for (let i = 0; i < signDoc.fee.amount.length; i++) {
    const feeCurrencyIndex = chainInfo.feeCurrencies.findIndex((fee: any) =>
      [fee.coinDenom, fee.coinMinimalDenom].includes(
        signDoc.fee.amount[i].denom,
      ),
    );

    if (feeCurrencyIndex < 0) {
      throw new Error(
        'FeeCurrency denom given is not a feeCurrency denom for this chain',
      );
    } else {
      const averageProp =
        chainInfo?.feeCurrencies?.[feeCurrencyIndex]?.gasPriceStep?.high;
      const fee =
        (averageProp ?? defaultGasPriceStep.high) * parseInt(signDoc.fee.gas);
      modifiedFeeSignDoc.fee.amount[i].denom =
        chainInfo?.feeCurrencies?.[feeCurrencyIndex]?.coinMinimalDenom;
      modifiedFeeSignDoc.fee.amount[i].amount = parseInt(
        fee.toString(),
      ).toString();
    }
  }
  ...
  for (let i = 0; i < authInfo.fee.amount.length; i++) {
    ...
    if (feeCurrencyIndex < 0) {
      ...
      const averageProp =
        chainInfo?.feeCurrencies?.[feeCurrencyIndex]?.gasPriceStep?.high;
      const fee =
        (averageProp ?? defaultGasPriceStep.high) *
        parseInt(authInfo.fee.gasLimit || authInfo.fee.gas);
      authInfo.fee.amount[i].denom =
        chainInfo?.feeCurrencies?.[feeCurrencyIndex]?.coinMinimalDenom;
