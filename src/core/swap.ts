...
export const getApproval = async ({ web3, fromTokenContractAddress, hdWallet, gasLimit, gasFeeResponse, contractData }: SwapMetaData) => {
  ...
  return await new Promise((resolve, reject) => {
    void (async () => {
      try {
        ...
        signPromise.then((signedTx: { rawTransaction: string }) => {
          void web3.eth.sendSignedTransaction(signedTx.rawTransaction)
...
export const checkAllowance = async ({ web3, fromToken, fromTokenContractAddress, routerAddress, amount, hdWallet }: SwapMetaData) => {
  const { ethereum } = hdWallet?.state.wallet;
  return await new Promise((resolve, reject) => {
    void (async () => {
      try {
        const contract = new web3.eth.Contract(contractABI, fromTokenContractAddress);
        const response = await contract.methods.allowance(ethereum.address, routerAddress).call();
        const tokenAmount = web3?.utils.toWei(String(Number(amount).toFixed(9)));
        ...
        if (Number(tokenAmount) > Number(allowance)) {
          ...
          const gasFeeResponse = await getGasPriceFor(fromToken?.chainDetails, web3);
          resolve({ isAllowance: false, contract, contractData: resp, tokens, gasLimit, gasFeeResponse });
        } else {
...
export const swapTokens = async ({ web3, fromToken, amount, routerAddress, quoteData, hdWallet, gasLimit, gasFeeResponse }: SwapMetaData) => {
  return await new Promise((resolve, reject) => {
    void (async () => {
      try {
        ...
        const tx = {
          ...
          value: isNative ? web3.utils.toWei(String(Number(amount).toFixed(fromToken?.contractDecimals)), 'ether') : '0',
          to: routerAddress,
          data: quoteData.data.data,
          gas: web3.utils.toHex(2 * Number(gasLimit)),
          gasPrice: web3.utils.toWei(String(gasFeeResponse.gasPrice.toFixed(9)), 'gwei')
        };
        const signPromise = web3?.eth.accounts.signTransaction(
          tx,
          String(ethereum.privateKey)
        );
        signPromise.then((signedTx: { rawTransaction: any }) => {
          void web3.eth.sendSignedTransaction(signedTx.rawTransaction)
