...
export const uintToHex = (value: Uint8Array) => `0x${Buffer.from(value).toString('hex')}`;
...
export const generateEthAddress = (mnemonic: string, index = 0): IAccountDetailWithChain => {
...
export const generateCosmosWallet = async (chainName: AddressChainNames, chainConfig: IIBCData, mnemonic: string,
  bip44HDPath: {
    account: number
    change: number
    addressIndex: number
  ...
  const privateKey = Mnemonic.generatePrivateKeyFromMasterSeed(masterSeed, path);
...
export const generateEvmosWallet = (ethereumAddress: string, chainConfig: IIBCData, mnemonic: string,
  bip44HDPath: {
    account: number
    change: number
    addressIndex: number
...
export const generateWalletFromMnemonic = async (mnemonic: string, trackingEventId: string): Promise<{ accounts: IAccountDetailWithChain[], mnemonic: string }> => {
  const bip44HDPath = {
    account: 0,
    change: 0,
    addressIndex: 0
  ...
  const cosmosAccounts = await Promise.all(
    cosmosChains.map(async (chain: AddressChainNames) => {
      return await generateCosmosWallet(chain, cosmosConfig[chain], mnemonic, bip44HDPath);
    })
  );

  const evmosWallet = generateEvmosWallet(ethereumWallet.address, cosmosConfig.evmos, mnemonic, bip44HDPath);

  const accounts: IAccountDetailWithChain[] = [ethereumWallet, ...cosmosAccounts, evmosWallet];
