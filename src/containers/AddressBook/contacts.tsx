...
import { CyDScrollView, CyDTextInput, CyDView } from '../../styles/tailwindStyles';
...
export function Contacts ({ route, navigation }) {
        ...
        for (const [chainName, listOfAddresses] of Object.entries(tempContactBook[contact].addresses)) {
          for (const address of listOfAddresses) {
            if (tempAddressDirectory[address]) {
              tempAddressDirectory[address] = [...tempAddressDirectory[address], tempContactBook[contact].name];
            } else {
  ...
  const removeContactFromAsyncStorage = async (contactName: string) => {
    delete contactBook[contactName];
    hideModal();
    setLoading(true);
  ...
  const deleteContact = (contactName: string) => {
    showModal('state', { type: 'warning', title: `${t('DELETE')} ${contactName}`, description: `${t('ARE_YOU_SURE_TO_DELETE')} ${contactName} ${t('FROM_YOUR_CONTACTS')} ?`, onSuccess: () => { void removeContactFromAsyncStorage(contactName); }, onFailure: hideModal });
  };

  const handleSearch = (text: string) => {
    if (text !== '') {
      const contactNamesByNames = fuseByNames.search(text).map(contact => contact.item);
      const contactNamesByAddresses: string[] = [];
      fuseByAddresses.search(text).map(contact => contact.item).forEach(address => {
        for (const name of addressDirectory[address]) {
          contactNamesByAddresses.push(name);
        }
      });
      const contactNames = [...new Set([...contactNamesByNames, ...contactNamesByAddresses])];
      ...
      <CyDScrollView className={'flex flex-col w-[100%]'}>
        <CyDView className={'flex flex-row justify-around mx-[15px]'}>
          <CyDTextInput
            ...
            autoCapitalize="none"
            autoCorrect={false}
            onChangeText={(value: string) => { setName(value); handleSearch(value); }}
        ...
        <CyDView className='items-center mx-[20px] mt-[-28px]'>
          <AddressProfile content={filteredContactBook} deleteContact={deleteContact} navigation={navigation}/>
        </CyDView>
      </CyDScrollView>
    </>
  );
