...
import { apiTimeout, getWeb3Endpoint, HdWalletContext, ActivityContext, PortfolioContext, TARGET_CARD_COSMOS_WALLET_ADDRESS, TARGET_CARD_EVMOS_WALLET_CORRESPONDING_EVM_ADDRESS, TARGET_CARD_EVM_WALLET_ADDRESS, TARGET_CARD_OSMOSIS_WALLET_ADDRESS, TARGET_CARD_JUNO_WALLET_ADDRESS } from '../../core/util';
import { Chain, CHAIN_ETH, CARD_CHAINS, ChainNames, COSMOS_CHAINS } from '../../constants/server';
...
import { ActivityReducerAction, ActivityStatus, ActivityType, DebitCardTransaction } from '../../reducers/activity_reducer';
...
export default function FundCardScreen (props) {
  ...
  async function fetchFromData (chain) {
    const chainHoldings = getCurrentChainHoldings(portfolioState.statePortfolio.tokenPortfolio, chain);
    const tokenList = [];
  ...
  function transferSentQuote (address: string, quote_uuid: string, txnHash: string) {
    axios.post(`${PORTFOLIO_HOST}/v1/card/mobile/transfer_sent`, {
      ...
      if (response.status === 200) {
        ...
        showModal('state', { type: 'success', title: '', description: 'Success, Your card funding is in progress and will be done within 5 mins!', onSuccess: hideModal, onFailure: hideModal });
      } else {
  ...
  const handleSuccessfulTransaction = async (result: any, analyticsData: any) => {
    ...
    showModal('state', { type: 'info', title: 'Transaction Hash', description: result.transactionHash, onSuccess: hideModal, onFailure: hideModal });
  ...
  const sendTransaction = async (payTokenModalParamsLocal: any) => {
    ...
    if (chainSelected != null) {
      if (chainSelected.chainName === ChainNames.ETH || chainSelected.chainName === ChainNames.EVMOS) {
        ...
        await sendNativeCoinOrTokenToAnyAddress(
          ...
          (chainSelected.chainName === ChainNames.ETH) ? TARGET_CARD_EVM_WALLET_ADDRESS : TARGET_CARD_EVMOS_WALLET_CORRESPONDING_EVM_ADDRESS,
  ...
  async function getQuote () {
    ...
    if (chainSelected?.chainName === ChainNames.ETH || chainSelected?.chainName === ChainNames.EVMOS) {
      ...
      axios.post(
        ...
        if (response?.data && response.status === 200 && response.data.status !== 'ERROR') {
          if (chainSelected != null) {
            ...
            quote.token_required = String(Number((quote.token_required)).toFixed(5));
            ...
            getGasPriceFor(chainSelected, web3RPCEndpoint)
              .then((gasFeeResponse) => {
                ...
                setLoading(false);
                estimateGasForNativeTransaction(hdWallet, chainSelected, fromTokenItem, quote.token_required, true, gasPrice, payTokenModal, globalContext);
              })
              .catch((gasFeeError) => {
          ...
          setErrorString('Unable to transfer at this point. Please try again later');
          setDisplayError(true);
          setLoading(false);
        }
      }).catch(function (error) {
      ...
      const quoteUrl = `${PORTFOLIO_HOST}/v1/card/mobile/quote_cosmos?address=${ethereum.address}&chain=${chainSelected?.backendName}&amount=${parseFloat(dollar)}&coin_id=${fromTokenItem?.coinGeckoId}`;
      axios.get(
        quoteUrl,
        { withCredentials: true, timeout: apiTimeout }
        ...
        if (response?.data && response.status === 200 && response.data.status !== 'ERROR') {
          if (chainSelected != null) {
            ...
            let retryCount = 0;
            const signer = await getCosmosSignerClient(chainSelected, hdWallet);
            while (retryCount < 3) {
              try {
                await estimateGasForCosmosTransaction(chainSelected, signer, amount, senderAddress[chainSelected.chainName], targetCardWalletAddress[chainSelected.chainName], cosmosTokenData[fromTokenItem.coinGeckoId], rpc[chainSelected.chainName], cosmosPayTokenModal, valueForUsd.toString());
              } catch (err) {
                if (retryCount < 3) {
                  retryCount += 1;
                  continue;
  ...
  const renderItem = (item: any) => {
    return (
              <DynamicTouchView sentry-label='fund-card-chain-item' dynamic dynamicWidth width={100} fD='row' mT={2} bR={15} pH={8} pV={8}
                  bGC={Colors.whiteColor} jC={'flex-start'} onPress={() => {
                    setDisplayQuote(false);
                    setDisplayError(false);
  ...
  const renderCoin = (item) => {
    return (
              <DynamicTouchView sentry-label='fund-card-chain-item' dynamic dynamicWidth width={100} fD='row' mT={2} bR={15} pH={8} pV={8}
                  bGC={Colors.whiteColor} jC={'flex-start'}
                  onPress={() => {
              ...
              >
                  <DynamicImage dynamic source={{ uri: item.logo_url }} width={25} height={25} />
                  <DynamicView dynamic dynamicWidth width={90} aLIT={'flex-start'}>
                      <CText dynamic fF={C.fontsName.FONT_BOLD} mL={8} fS={16} color={Colors.secondaryTextColor}>{item.name} - {item.actualBalance} (${item.totalValue})</CText>
                      <CText dynamic fF={C.fontsName.FONT_BOLD} mL={8} fS={12} color={Colors.subTextColor}>{item.symbol}</CText>
                  </DynamicView>
              </DynamicTouchView>
    );
             ...
             {<>
                 <BottomTokenCardConfirm
                     ...
                     onCancelPress={() => {
                       setPayTokenBottomConfirm(false);
                       setLowBalance(false);
                       analytics().logEvent('cancel_trandfe_token', { from: ethereum.address });
                 ...
                 <KeyboardAwareScrollView extraScrollHeight={100}>
                     <DynamicView dynamic dynamicWidth dynamicHeight height={100} width={100} jC='flex-start' aLIT={'flex-start'}>
                         <CText dynamic fF={C.fontsName.FONT_BOLD} fS={28} mL={25} mT={10} color={Colors.primaryTextColor}>{t('FUND_YOUR_CARD')}</CText>
                         <DynamicView dynamic dynamicWidth dynamicHeight height={100} width={95} mT={10} pH={10} jC='flex-start' aLIT={'flex-start'}>
                             <Dropdown
                                 ref={ref2}
                                 ...
                                 renderLeftIcon={() => (
                                     <DynamicImage dynamic dynamicWidth height={20} width={30} resizemode='contain'
                                         source={icon} />
                                 )}
                             />
                             <Dropdown
                                ...
                                renderLeftIcon={() => (
                                    <DynamicImage dynamic dynamicWidth height={20} width={30} source={{ uri: fromTokenItem.logoUrl }} />
                                )}
                            />
                            <CText dynamic fF={C.fontsName.FONT_BOLD} fS={18} mL={10} tA={'left'} mT={40} color={Colors.primaryTextColor}>{t('HOW_MANY_DOLLAR')}</CText>
                            <WebsiteInput
                            ...
                            /> */}
                            <DynamicView dynamic dynamicHeight dynamicWidth height={0.08} mL={10} width={95} bGC={'black'}></DynamicView>
                            {displayError
                              ? <DynamicView dynamic dynamicWidth width={95} jC='center' mT={10} aLIT={'flex-start'} pH={10} pV={10}>
                                    <CText dynamic tA={'left'} fF={C.fontsName.FONT_BOLD} fS={15} color={Colors.secondaryTextColor}> {errorString} </CText>
                              ...
                              : <></>}
                            <DynamicView dynamic mL={10} dynamicWidth dynamicHeight height={15} width={100} jC='center' aLIT={'center'} >
                                <ButtonWithOutImage sentry-label='card-get-quote' wT={100} bR={10} fE={C.fontsName.FONT_BOLD} hE={45} mT={25} bG={Colors.appColor}
                                    vC={Colors.appColor} text={t('FUND CARD')}
                                    onPress={() => { getQuote(); }
                                    } indicator={loading} />
                            </DynamicView>
                        </DynamicView>
