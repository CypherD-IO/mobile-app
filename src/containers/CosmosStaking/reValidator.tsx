...
import { CyDView, CyDFlatList, CyDText, CyDImage, CyDTouchView, CyDTextInput } from '../../styles/tailwindStyles';
import { CosmosActionType, CosmosStakingContext, IAllValidators } from '../../reducers/cosmosStakingReducer';
import { convertFromUnitAmount, convertNumberToShortHandNotation } from '../../core/util';
...
export default function CosmosSelectReValidator ({ route, navigation }) {
    ...
    {validatorData.name !== item.name && <CyDTouchView onPress={() => {
      ...
      <CyDView className={'my-[24px] mx-[20px]'}>
        <CyDView className={'flex flex-row justify-between'}>
          <CyDText className={'text-[16px] font-bold text-secondaryTextColor font-nunito w-1/2'} numberOfLines={2}>{item.name}</CyDText>
          {item.balance.toString() !== '0' &&
          <CyDText className={'ml-[10px] text-[16px] font-bold text-secondaryTextColor font-nunito'}>{`${convertFromUnitAmount(item.balance.toString(), tokenData.contractDecimals)} ${tokenData.name}`}</CyDText>}
        </CyDView>
        <CyDView className={'flex flex-row item-center mt-[8px]'}>
          ...
          <CyDImage source={AppImages.COINS} className={'ml-[20px] w-[20px] h-[16px]'} />
          <CyDText className={'ml-[4px] font-nunito text-subTextColor'}>{convertNumberToShortHandNotation(parseFloat(convertFromUnitAmount(item.tokens.toString(), tokenData.contractDecimals)))}</CyDText>
        </CyDView>
      </CyDView>
  ...
  return (
    <CyDView className={'bg-white h-full w-full'}>
      ...
      <CyDView className={'bg-[#F6F6F6] p-[8px] mt-[10px] mx-[20px] rounded-[8px] flex flex-row items-center'}>
        <CyDTextInput className={'ml-[4px] bg-[#F6F6F6] text-[20px] h-[40px] w-[55%] font-nunito'}
      ...
      <CyDFlatList data={queryData} renderItem={memoizedValue} keyExtractor={(item) => item.address}/>
    </CyDView>
  );
}
