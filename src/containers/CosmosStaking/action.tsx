...
export default function CosmosAction ({ route, navigation }) {
  ...
  return (
    <CyDScrollView className={'bg-white h-full w-full px-[20px]'}>
      ...
      <CyDText className={'text-[24px] mt-[24px] font-nunito text-secondaryTextColor font-extrabold text-start'}>{validatorData.name}</CyDText>
      <CyDView className={'flex flex-row items-center justify-start mt-[24px]'}>
        <CyDImage source={AppImages.GIFT_BOX_PNG} className={'w-[20px] h-[20px]'}/>
      ...
      <CyDView className={'flex flex-row items-center justify-start mt-[12px]'}>
        ...
        <CyDText className={'ml-[4px] text-[14px] font-nunito ttext-secondaryTextColor font-bold text-center'}>{`${convertNumberToShortHandNotation(convertFromUnitAmount(validatorData.tokens, tokenData.contractDecimals))} ${tokenData.name}`}</CyDText>
      </CyDView>
      ...
      <CyDView className={'bg-[#FFE3DB4D] my-[32px] py-[16px] px-[20px] rounded-[8px] flex flex-row'}>
        ...
        <CyDText className={'ml-[16px] font-nunito text-[13px] text-primaryTextColor w-10/12 font-semibold'}>{`${t('Once you undelegate your staked')} ${tokenData.name}${t(', you will need to wait 21 days for your tokens to be liquid')}`}</CyDText>
      </CyDView>
      ...
      {CosmosActionType.DELEGATE === from && <CyDView className={'mt-[20px] mb-[40px] flex flex-row justify-between items-center'}>
      ...
      {from === CosmosActionType.REDELEGATE && <>
        <CyDText className={'font-nunito text-[16px] mt-[10px] text-primaryTextColor'}>{t('Validator to Redelegate')}</CyDText>
        <CyDTouchView className={'bg-inputBackgroundColor p-[8px] rounded-[8px] h-[60px] flex flex-row items-center'}
        onPress={() => {
      ...
      <CyDText className={'mt-[20px] font-nunito text-[16px] text-primaryTextColor'}>{`${t('Amount to ')}${from}`}</CyDText>

      <CyDView className={'bg-inputBackgroundColor p-[8px] mt-[10px] h-[60px] rounded-[8px] flex flex-row items-center'}>
        <CyDTextInput className={'ml-[4px] bg-inputBackgroundColor text-[16px] w-[63%] font-nunito text-primaryTextColor'}
      ...
      <CyDView className={'bg-inputBackgroundColor p-[10px] h-[60px] rounded-[8px] flex flex-row items-center'}>
        <CyDTextInput className={'ml-[4px] bg-inputBackgroundColor text-[16px] w-7/12 font-nunito'}
                      onChangeText={(text) => {
                        setMemo(text);
                      }}
