  ...
  const BrowserView = ({ chain }: {chain: string}) => {
    ...
    switch (chain) {
      ...
      case ChainBackendNames.OPTIMISM:
        uri = tokenData.name === 'Optimism Mainnet Ether' ? `https://optimistic.etherscan.io/address/${ethereum.address}` : `https://optimistic.etherscan.io/address/${ethereum.address}#tokentxns`;
        break;
    ...
    return <>
            <CyDTouchView sentry-label='txn-detail-explorer' className={'flex flex-row py-[8px] justify-between'} onPress={() => {
              navigation.navigate(screenTitle.TRANS_DETAIL, {
                ...
                <CyDView className={'flex flex-row h-[54px] rounded-[20px] items-center justify-between'}>
                    <CyDView className={clsx('flex flex-row h-[25px] w-[32px] items-center justify-center bg-paleGrey rounded-[20px]', {
                      'bg-paleGrey': item?.status,
                      'bg-babyPink': !item?.status
                    })}>
                        <CyDFastImage className={'h-[32px] w-[32px]'}
                    ...
                    <CyDView className={'w-[140px] h-[54px] items-start flex flex-column justify-center px-[8px] mt-[-8px]'}>
                        ...
                        <CyDText className={'font-semibold text-[13px] text-subTextColor'}>{calculateTime(item?.date)}</CyDText>
                    </CyDView>
                </CyDView>
                <CyDView className={'h-[54px] flex flex-row justify-center'}>
                    <CyDView className={'flex flex-column w-[160px] h-[54px] items-end justify-center'}>
                        <CyDText className={clsx('text-right font-semibold text-[18px]', { 'text-primaryTextColor': item?.status && item?.direction === 'TO', 'text-toastColor': item?.status && item?.direction === 'TO', 'text-pinkCyD': !item?.status })}>{item?.direction === 'TO' ? '+' : '-'}{item?.tokenQuantity.toFixed(6)} {item?.tokenSymbol}</CyDText>
                        <CyDText className={'text-[16px] text-tokenPriceColor'}>{(item?.tokenQuantity * Number(tokenData.price)).toFixed(2).toString() + ' USD'}</CyDText>
