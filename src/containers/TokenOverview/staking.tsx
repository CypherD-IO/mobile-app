...
export default function TokenStaking ({ tokenData, navigation }: { tokenData: TokenMeta, navigation: { navigate: (screen: string, {}: any) => void } }) {
  ...
  const [stakingVariables, setStakingVariables] = useState({ ...initialStakeVariables });
  ...
  const isStakingDataEmpty = () => {
    return isStakingContextDispatched() && stakingVariables.totalClaimableRewards === '0' && stakingVariables.availableToStake === '0' && stakingVariables.currentlyStaked === '0' && stakingVariables.totalUnboundings === '0';
  };

  const isStakingContextDispatched = (): boolean => {
    return (tokenData.chainDetails.backendName !== ChainBackendNames.EVMOS && cosmosStaking.cosmosStakingState.allValidatorsListState !== COSMOS_STAKING_EMPTY) || (tokenData.chainDetails.backendName === ChainBackendNames.EVMOS && stakingValidators.stateStaking.allValidatorsListState !== STAKING_EMPTY);
  };

  useEffect(() => {
    if (isStakingContextDispatched()) {
      const defaultUnboundingPeriod = (tokenData.chainDetails.backendName === ChainBackendNames.EVMOS || tokenData.chainDetails.backendName === ChainBackendNames.STARGAZE || tokenData.chainDetails.backendName === ChainBackendNames.NOBLE) ? 14 : 21;
      const variables = {
        totalClaimableRewards: '0',
        availableToStake: '0',
        currentlyStaked: '0',
      ...
      if (tokenData.chainDetails.backendName !== ChainBackendNames.EVMOS) {
        setUnboundingPeriodInDays(cosmosStaking.unboundingPeriodInDays ?? defaultUnboundingPeriod);
        if ((cosmosStaking.cosmosStakingState.reward).toString() !== '0' || (cosmosStaking.cosmosStakingState.userValidators.size > 0)) {
          variables.totalClaimableRewards = (`${convertFromUnitAmount((cosmosStaking.cosmosStakingState.reward).toString(), tokenData.contractDecimals, 6)}`);
        }
        if ((cosmosStaking.cosmosStakingState.balance).toString() !== '0') {
          variables.availableToStake = (`${convertFromUnitAmount((cosmosStaking.cosmosStakingState.balance).toString(), tokenData.contractDecimals, 6)}`);
        }
        if (cosmosStaking.cosmosStakingState.stakedBalance.toString() !== '0') {
          variables.currentlyStaked = (`${convertFromUnitAmount((cosmosStaking.cosmosStakingState.stakedBalance).toString(), tokenData.contractDecimals, 6)}`);
        }
        if (cosmosStaking.cosmosStakingState.unBoundingBalance.toString() !== '0') {
          variables.totalUnboundings = (`${convertFromUnitAmount((cosmosStaking.cosmosStakingState.unBoundingBalance).toString(), tokenData.contractDecimals, 6)}`);
        }
      } else {
        setUnboundingPeriodInDays(stakingValidators.unboundingPeriodIndDays ?? defaultUnboundingPeriod);
        if (!isBigIntZero(stakingValidators.stateStaking.totalReward)) {
          variables.totalClaimableRewards = (`${convertToEvmosFromAevmos(stakingValidators.stateStaking.totalReward).toFixed(6)}`);
        }
        if (!isBigIntZero(stakingValidators.stateStaking.unStakedBalance)) {
          variables.availableToStake = (`${convertToEvmosFromAevmos(stakingValidators.stateStaking.unStakedBalance).toFixed(6)}`);
        }
        if (!isBigIntZero(stakingValidators.stateStaking.totalStakedBalance)) {
          variables.currentlyStaked = `${convertToEvmosFromAevmos(stakingValidators.stateStaking.totalStakedBalance).toFixed(6)}`;
        }
        if (!isBigIntZero(stakingValidators.stateStaking.unBoundingTotal)) {
          variables.totalUnboundings = (`${convertToEvmosFromAevmos(stakingValidators.stateStaking.unBoundingTotal).toFixed(6)}`);
        }
      }
  ...
  const onPressClaim = async (type: CosmosActionType): Promise<void> => {
    setLoading(true);
    if (type === CosmosActionType.TRANSACTION && parseFloat(gasFee.toFixed(6)) > parseFloat(stakingVariables.availableToStake)) {
      setLoading(false);
      setSignModalVisible(false);
      showNoGasFeeModal();
    } else {
    ...
    return (
      <CyDView className={'flex flex-row justify-center'}>
        <CyDView className={'mt-[15%]'}>
          <CyDImage source={AppImages.STAKING_EMPTY_ILLUSTRATION} className='h-[250px] w-[350px]' resizeMode='contain' />
    ...
    <CyDView>
                  ...
                  <CyDView>
                      ...
                      <CyDView className={'flex flex-row mt-[40px]'}>
                          ...
                          <CyDView className={' flex flex-row mt-[3px]'}>
                              ...
                              <CyDText className={'ml-[10px] text-[18px] font-bold'}>{stakingVariables.totalClaimableRewards}</CyDText>
                          </CyDView>
                      </CyDView>
                      ...
                      {isBasicCosmosChain(tokenData.chainDetails.backendName) && <CyDView className={'flex flex-col mt-[40px] w-[100%]'}>
                          ...
                          }} loading={actionType === CosmosActionType.RESTAKE && loading} loaderStyle={styles.loaderStyle} title={t<string>('RESTAKE')} style={'py-[5%]'} />
                          <Button onPress={async () => {
                            setActionType(CosmosActionType.CLAIM);
                            await onPressClaim(CosmosActionType.SIMULATION);
                          }} title={t<string>('CLAIM')} type={'secondary'} style={'py-[5%] mt-[15px]'} loading={actionType === CosmosActionType.CLAIM && loading} loaderStyle={styles.loaderStyle} />
                      </CyDView>}
                      {tokenData.chainDetails.backendName === ChainBackendNames.EVMOS && <CyDView className={'flex flex-col mt-[24px] w-[100%]'}>
                          ...
                          }} title={t<string>('RESTAKE')} style={'py-[5%]'} loading={actionType === CosmosActionType.RESTAKE && loading} loaderStyle={styles.loaderStyle} />
                          <Button onPress={async () => {
                            // setClaimModal(false);
                            setActionType(CosmosActionType.CLAIM);
              ...
              <CyDView className={'bg-white p-[25px] pb-[30px] rounded-[20px] relative'}>
                  ...
                  <CyDView className={'flex flex-row mt-[40px]'}>
                      ...
                      <CyDView className={' flex flex-row mt-[3px]'}>
                          ...
                          <CyDText className={' font-bold ml-[5px] text-[18px] text-center text-secondaryTextColor'}>{stakingVariables.totalClaimableRewards}</CyDText>
                      </CyDView>
                  </CyDView>
                  ...
                  <CyDView className={'flex flex-col mt-[30px] w-[100%]'}>
                      <Button onPress={async () => {
                        isBasicCosmosChain(tokenData.chainDetails.backendName)
                          ...
                          : await finalTxn();
                      }} title={t<string>('APPROVE')} style={'py-[5%]'} loading={loading} loaderStyle={styles.loaderHeight30} />
          ...
          <CyDModalLayout setModalVisible={setReStakeModalVisible} isModalVisible={reStakeModalVisible} style={styles.modalLayout} animationIn={'slideInUp'} animationOut={'slideOutDown'}>
              <CyDView className={'bg-white p-[25px] pb-[30px] rounded-[20px] relative'}>
                  ...
                  <CyDView className={'flex flex-col mt-[30px] w-[100%]'}>
                      ...
                      }} title={t<string>('APPROVE')} style={'py-[5%] min-h-[60px]'} loading={loading} loaderStyle={styles.loaderHeight30} />
          ...
          {pageLoading
            ...
            : <CyDScrollView className={'h-[90%]'} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} /> }>
              ...
              {stakingVariables.availableToStake !== '0' && (stakingVariables.currentlyStaked !== '0' || stakingVariables.totalClaimableRewards !== '0' || stakingVariables.totalUnboundings !== '0') &&
                  <CyDView>
                      <CyDView className={'my-[25px] mx-[30px] flex flex-row items-center justify-between'}>
                          <CyDView>
              ...
              {stakingVariables.availableToStake !== '0' && stakingVariables.currentlyStaked === '0' && stakingVariables.totalClaimableRewards === '0' && stakingVariables.totalUnboundings === '0' &&
                <CyDView className={'flex justify-center items-center mt-[80px]'}>
                  <CyDImage source={AppImages.STAKING_EMPTY_ILLUSTRATION} className='h-[250px] w-[350px]' resizeMode='contain' />
                  <CyDText className={'text-center w-3/4  text-[20px] font-semibold text-secondaryTextColor'}>{`${t<string>('STAKE_YOUR_TEXT')} ${tokenData.name.toLowerCase()} ${t<string>('WITH_US_TEXT')}`}</CyDText>
                  <Button onPress={() => {
                    isSignableTransaction(ActivityType.STAKE, onStake);
                  }} title={t<string>('STAKE_NOW')} isPrivateKeyDependent = {true} style={'w-1/2 mt-[20px] mb-[30px] p-[4%]'} />
                  <CyDView>
                    <CyDView className='flex flex-row'>

                        <CyDText className={'text-subTextColor  font-medium text-[15.5px]'}>{`${t<string>('AVAILABLE_TO_STAKE')}: `}</CyDText>
                        <CyDTokenAmount
                            className={'text-primaryTextColor font-bold text-[15.5px] ml-[2px]'}
                            decimalPlaces={5}
                          >
                              {stakingVariables.availableToStake}
                          </CyDTokenAmount>
                          <CyDText
                            className={'text-primaryTextColor font-bold text-[15.5px] ml-[5px]'}>{tokenData.name}
                          </CyDText>
                    </CyDView>
                  </CyDView>
                </CyDView>
              }
              {(stakingVariables.totalClaimableRewards !== '0' || stakingVariables.currentlyStaked !== '0' || stakingVariables.totalUnboundings !== '0') && <CyDView className={'bg-[#F6F7FF] rounded-[8px] px-[46px] py-[16px] mx-[16px] my-[18px] flex flex-col items-center'}>
                          ...
                          <CyDView className={'flex flex-row mt-[12px] '}>
                              ...
                              <CyDText className={'font-[16px] text-primaryTextColor font-bold '}>{unboundingPeriodInDays} {t('DAYS')}</CyDText>
                          </CyDView>
                      </CyDView>
              }
