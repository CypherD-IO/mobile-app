...
export default function Unboundings({ route, navigation }) {
  ...
  const Item = ({ item, index }) => (
    <DynamicView
      ...
      mR={12}
      mB={
        index === stakingValidators.stateStaking.unBoundingsList.length - 1
          ? 50
          : 30
      }>
      <DynamicView dynamic fD={'row'} dynamicWidth width={100}>
        <CText
          ...
          {(item.balance * 10 ** -18).toFixed(3) + ' EVMOS'}
        </CText>
        <CText
          dynamic
          fF={C.fontsName.FONT_SEMI_BOLD}
          ...
          {convertDate(item.completion_time)}
        </CText>
      </DynamicView>
      <DynamicView
        dynamic
        ...
        <CText
          ...
          tA={'left'}
          color={Colors.subTextColor}>
          {daysRemaining(item.completion_time) + ' days remaining'}
        </CText>
      </DynamicView>
      <DynamicView
        dynamic
        ...
        style={{ flex: 1 }}>
        <Progress.Bar
          progress={progressPercentage(item.completion_time)}
          width={350}
          height={5}
