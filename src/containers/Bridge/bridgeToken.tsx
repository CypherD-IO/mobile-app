...
export default function BridgeTokebScreen(props) {
  ...
  const [portfolioRedirection, setPortfolioRedirection] =
  ...
  async function bridgeQuote(
    _toValueItem: any = undefined,
    _toToken: any = undefined,
  ) {
    ...
    if (
      parseFloat(fromAddedValue) <= parseFloat(threshold) ||
      fromAddedValue === ''
    ) {
      setError({
        displayError: true,
        errorMsg: 'Minimum amount is ' + parseFloat(threshold).toFixed(3),
      });
      return;
    }
    ...
    if (_toValueItem !== undefined) {
      toValueItem_final = _toValueItem;
    }
    if (_toToken !== undefined) {
      toToken_final = _toToken;
    }
    ...
    await axios
      ...
      .then(res => {
        ...
        if (res.data.status == 'ERROR') {
          showModal('state', {
            type: 'error',
            title: '',
            description: `${res.data.message} Please try transfering another pair.`,
        ...
        const _data = {
          ...
          totalValueSentUSD: (
            parseFloat(fromAddedValue) * parseFloat(fromTokenItem.price)
          ).toFixed(6),
          sentTokenSymbol: fromTokenItem.symbol,
          fromNativeTokenSymbol: chainSelected.symbol,
          gasFeeNative: parseFloat(
            res.data.estimated_gas_fee_native_token,
          ).toFixed(6),
      ...
      .catch(error => {
        ...
        showModal('state', {
          ...
          title: '',
          description:
            'Unable to get quote for your request. Please contact Cypher customer support.',
          onSuccess: onModalHide,
          onFailure: hideModal,
        });
        Sentry.captureException(error);
  ...
  async function fetchFromData(chain) {
    ...
    const portfolio: WalletHoldings =
      portf !== null ? (portf.portfolio as unknown as WalletHoldings) : null;
  ...
  const CoinItem = ({ item, onChange }) => {
    return (
      <DynamicTouchView
        sentry-label='bridge-choose-coin'
        dynamic
        dynamicWidth
        width={100}
        fD='row'
        ...
        pH={8}
        pV={8}
        bGC={value === item.name ? Colors.warningYellow : Colors.whiteColor}
        jC={'flex-start'}
        onPress={() => {
          onChange(item);
        }}>
  ...
  const renderFromToken = item => {
    return item.isVerified ? (
      <DynamicTouchView
        sentry-label='bridge-from-token'
        dynamic
        dynamicWidth
        width={100}
        fD='row'
  ...
  function transferSentQuote(
    ...
    axios
      .post(
        `${PORTFOLIO_HOST}/v1/bridge/transfer_sent`,
        {
          address,
          quote_uuid,
          trk_data: { device: Platform.OS },
      ...
      <DynamicScrollView
        ...
        <DynamicView dynamic pV={10}>
          <DynamicView
            ...
            <Input
              ...
              placeholder={
                tokenAvailable
                  ? 'Enter Amount >' +
                    parseFloat(threshold).toFixed(3) +
                    ' ' +
                    fromTokenItem.symbol
                  : 'NO TOKEN AVAILABLE'
              }
          ...
          <DynamicView
            ...
            <AppButton
              mT={20}
              bGC={!tokenAvailable && '#dddddd'}
              text={t('GET_QUOTE')}
              indicator={appButtonState.inProgress}
              disable={appButtonState.inProgress || !tokenAvailable}
              onPress={bridgeQuote}
              mB={undefined}
            />
          </DynamicView>
