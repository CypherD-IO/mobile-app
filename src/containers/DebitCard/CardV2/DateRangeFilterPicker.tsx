...
const DatePresetButton = ({
  ...
  return (
    <CyDTouchView
      className={clsx(
        'px-[10px] mx-[3px] py-[5px] my-[5px] rounded-[8px] border border-greyButtonBackgroundColor',
        { 'bg-buttonColor': isActive },
      )}
...
const DateRangeFilterPicker = ({
  ...
  const Presets = useMemo(() => {
    return PRESET_OFFSET_DAYS.map((offset, index) => {
      ...
      const fromDateIsEqual =
        moment().subtract(offset, 'days').format(formatText) ===
        moment(dateRange.fromDate).format(formatText);
      const toDateIsEqual =
        moment().format(formatText) ===
        moment(dateRange.toDate).format(formatText);
      return (
        <DatePresetButton
          key={index}
          presetOffset={offset}
          setDateRange={setDateRange}
          isActive={fromDateIsEqual && toDateIsEqual}
        />
      );
    });
