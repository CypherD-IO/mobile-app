  ...
  const fundCard = async () => {
    ...
    if (chainName === ChainNames.ETH || chainName === ChainNames.EVMOS) {
      ...
      if (response?.data && !response.isError) {
        ...
        showModal('state', { type: 'error', title: '', description: response.error.response.data.errors[0]?.message ?? t('UNABLE_TO_TRANSFER'), onSuccess: hideModal, onFailure: hideModal });
        setLoading(false);
      }
    } else if (COSMOS_CHAINS.includes(chainName)) {
      try {
