  ...
  const activateCard = async () => {
    ...
    try {
      ...
      if (!response.isError) {
        ...
        showModal('state', { type: 'error', title: t('VERIFICATION_FAILED'), description: response.error.response.data.errors[0]?.message ?? t('INVALID_DETAILS'), onSuccess: () => onModalHide(), onFailure: () => onModalHide() });
      }
    } catch (e: any) {
