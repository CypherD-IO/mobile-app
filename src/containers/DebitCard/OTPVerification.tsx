...
import { CyDView, CyDText, CyDTouchView, CyDImage, CyDSafeAreaView, CyDTextInput, CyDKeyboardAvoidingView } from '../../styles/tailwindStyles';
...
export default function OTPVerificationScreen ({ navigation }) {
  ...
  const getProfile = async () => {
    ...
    try {
      ...
      const profileData = {
        ...
        updatedPhoneNumber: data.phone.split(selectedCountryWithDialCode.dial_code).pop(),
  ...
  const verifyOTPByType = async (otp) => {
    ...
    try {
      ...
      showModal('state', { type: 'error', title: t('VERIFICATION_FAILED'), description: concatErrorMessagesFromArray(e.response.data.errors), onSuccess: () => onModalHide(), onFailure: () => onModalHide() });
      Sentry.captureException(e);
    }
  };
  ...
  const triggerOTP = async (type: string) => {
    ...
    try {
      ...
      showModal('state', { type: 'error', title: t('OTP_TRIGGER_FAILED'), description: concatErrorMessagesFromArray(e.response.data.errors), onSuccess: hideModal, onFailure: hideModal });
      Sentry.captureException(e);
    }
  };
  ...
  const updateUserDetails = async (detail: string) => {
    ...
    if (detail === OTPType.EMAIL) {
      ...
      phoneNumberWithoutSpecialCharacters = formData.updatedPhoneNumber.replace(/[- +()#*;,.<>{}[\]\\/]/gi, '');
      phoneNumberWithoutDialCodes = phoneNumberWithoutSpecialCharacters.replace(formData.dialCode, '');
    ...
    try {
      ...
      if (detail === OTPType.PHONE) {
        setFormData({ ...formData, phoneNumber: formData.dialCode + phoneNumberWithoutDialCodes, updatedPhoneNumber: phoneNumberWithoutDialCodes });
      ...
      <CyDKeyboardAvoidingView behavior={isAndroid() ? 'height' : 'padding'} className='flex flex-col justify-end h-full'>
          <CyDView className={'bg-white h-[250px] rounded-t-[20px]'}>
              ...
              <CyDView className={clsx('h-[50px] ml-[30px] mt-[20px] border-[1px] border-inputBorderColor rounded-[5px] w-[85%] flex flex-row')}>
                  <CyDView className={'w-4/12 border-r-[1px] border-[#EBEBEB] bg-white py-[13px] rounded-l-[16px] flex items-center'}>
                      <CyDView className={'mt-[-4px] ml-[-55px]'}>
                          <CyDText className={'text-[33px] mt-[-6px]'}>{formData.countryFlag}</CyDText>
                      </CyDView>
                      <CyDView className={'mt-[-20px] ml-[45px]'}>
                          <CyDText className={'text-[13px] font-extrabold text-center'}>{formData.dialCode}</CyDText>
                      </CyDView>
                  </CyDView>
                  <CyDView className={'flex flex-row items-center w-8/12'}>
                      <CyDView className={'flex flex-row items-center'}>
                          <CyDTextInput className={clsx('text-center text-black font-nunito text-[16px] ml-[8px]', { 'mt-[-8px]': isAndroid() })}
                              ...
                              onChangeText={(value) => setFormData({ ...formData, updatedPhoneNumber: value })}
                              placeholder='Phone Number' />
                      </CyDView>
                  </CyDView>
              </CyDView>
              <CyDTouchView onPress={() => { setChangeNumberModalVisible(false); void updateUserDetails(OTPType.PHONE); }}
              ...
              <CyDView className={'bg-white h-[250px] rounded-t-[20px]'}>
                  ...
                  <CyDView className={'mt-[20px] flex flex-row justify-center'}>
                      <CyDTextInput className={'ml-[4px] border-[1px] border-inputBorderColor rounded-[5px] p-[12px] text-[18px] w-[85%] font-nunito text-primaryTextColor'}
                          ...
                          autoCorrect={false}
                          placeholderTextColor={'#C5C5C5'}
                          onChangeText={(value) => setFormData({ ...formData, updatedEmail: value })}
                          placeholder='Email Id' />
                  </CyDView>
                  <CyDTouchView onPress={() => { setChangeEmailModalVisible(false); void updateUserDetails(OTPType.EMAIL); }}
                      className={'bg-appColor py-[20px] flex flex-row justify-center items-center rounded-[12px] justify-around w-[86%] mx-auto mt-[25px]'}>
                      <CyDText className={'text-[16px] text-center font-bold'}>{t<string>('UPDATE_INIT_CAPS')}</CyDText>
                  </CyDTouchView>
              </CyDView>
            ...
            : <CyDSafeAreaView className={'h-full bg-white '}>
              <CyDView className={'flex flex-row mt-[30px]'}>
                  <CyDView className={'w-[6%] h-full'}>
                      <CyDView className={'absolute ml-[21px] rounded-[50px] w-[14px] h-[14px] bg-appColor'}></CyDView>
                      <CyDView className={clsx('absolute mt-[14px] ml-[26px] w-[3px] bg-paleGrey', { 'h-[230px]': !isPhoneOTPVerified, 'h-[141px]': isPhoneOTPVerified, 'h-[248px]': invalidOTP !== '' })}></CyDView>
                      <CyDView className={clsx('absolute ml-[21px] rounded-[50px] w-[14px] h-[14px]', { 'mt-[243px] border-[1px] border-appColor': !isPhoneOTPVerified, 'mt-[148px] bg-appColor': isPhoneOTPVerified, 'mt-[260px]': invalidOTP !== '' })}></CyDView>
                  </CyDView>
                  <CyDView className={'w-[93%]'}>
                      <CyDView className={'mx-[22px]'}>
                          ...
                          {!isPhoneOTPVerified && <CyDView>
                              <CyDText className={'text-[17px] mt-[12px] font-semibold'}>
                                  {`${t<string>('SENT_AUTHENTICATION_CODE_TO')} ${formData.phoneNumber} ${t<string>('THIS_NUMBER')}`}
                              </CyDText>
                              {!verifyingOTP && <CyDView className={'flex flex-row justify-center'}>
                                  <OtpInput pinCount={4} getOtp={async (otp) => await getOTP(otp)}></OtpInput>
                              </CyDView>}
                              {verifyingOTP && <CyDView className={'flex items-center justify-between  h-[65px]'}>
                                <LottieView source={AppImages.LOADER_TRANSPARENT}
                                    autoPlay
                                    loop
                                    style={{ height: 40 }}
                                />
                              ...
                              {invalidOTP !== '' && <CyDView>
                                  <CyDText className={'text-redOffColor font-bold text-center'}>{invalidOTP}</CyDText>
                              </CyDView>}
                              <CyDView className={'flex flex-row justify-between mt-[12px] mr-[12px]'}>
                                  <CyDTouchView className={'flex flex-row items-center'} disabled={resendingCode || resendInterval !== 0} onPress={() => { void resendCode(OTPType.PHONE); }}>
                                        <CyDText className={'font-bold underline decoration-solid underline-offset-4'}>{t<string>('RESEND_CODE_INIT_CAPS')}</CyDText>
                                        {resendingCode && <LottieView source={AppImages.LOADER_TRANSPARENT}
                                            autoPlay
                                            loop
                                            style={{ height: 25 }}
                                        />}
                                  ...
                                  <CyDTouchView onPress={() => { setChangeNumberModalVisible(true); } }>
                                      <CyDText className={'font-bold underline decoration-solid underline-offset-4'}>{t<string>('CHANGE_NUMBER_INIT_CAPS')}</CyDText>
                                  </CyDTouchView>
                              </CyDView>
                          </CyDView>}
                          {isPhoneOTPVerified && <CyDView className={'flex flex-row justify-between items-center bg-paleDarkGreen mt-[10px] px-[8px] py-[10px] rounded-[5px]'}>
                              <CyDText className={'text-[16px] pl-[8px] font-bold'}>
                                  {formData.phoneNumber}
                              </CyDText>
                              <CyDImage source={AppImages.DARK_GREEN_BACKGROUND_TICK} />
                          </CyDView>}
                          <CyDView className={'h-[1px] w-[100%] bg-portfolioBorderColor mt-[25px]'}></CyDView>
                      </CyDView>
                      <CyDView className={'mx-[22px]'}>
                          <CyDText className={clsx('text-[26px] font-bold', { 'mt-[23px]': isIOS(), 'mt-[33px]': isAndroid() })}>
                              {t<string>('VERIFY_EMAIL_ID_HEADING')}
                          </CyDText>
                          {isPhoneOTPVerified && <CyDView className={'w-full'}>
                              {!isEmailOTPVerified && <CyDView>
                                  <CyDText className={'text-[17px] mt-[12px] font-semibold'}>
                                      {`${t<string>('SENT_AUTHENTICATION_CODE_TO')}`} {formData.email}
                                  </CyDText>
                                      {!verifyingOTP && <CyDView className={'flex flex-row justify-center'}>
                                          <OtpInput pinCount={4} getOtp={async (otp) => await getOTP(otp)}></OtpInput>
                                      </CyDView>}
                                      {verifyingOTP && <CyDView className={'flex items-center justify-between'}>
                                          <LottieView source={AppImages.LOADER_TRANSPARENT}
                                              autoPlay
                                              loop
                                              style={{ height: 40 }}
                                  ...
                                  <CyDView className={'flex flex-row justify-between items-center mt-[12px] mr-[12px]'}>
                                      <CyDTouchView className={'flex flex-row items-center'} disabled={resendingCode || resendInterval !== 0} onPress={() => { void resendCode(OTPType.EMAIL); }}>
                                          <CyDText className={'font-bold underline decoration-solid underline-offset-4'}>{t<string>('RESEND_CODE_INIT_CAPS')}</CyDText>
                                          {resendingCode && <LottieView source={AppImages.LOADER_TRANSPARENT}
                                            autoPlay
                                            loop
                                      ...
                                      <CyDTouchView onPress={() => { setChangeEmailModalVisible(true); } }>
                                          <CyDText className={'font-bold underline decoration-solid underline-offset-4'}>{t<string>('CHANGE_EMAIL_INIT_CAPS')}</CyDText>
                                      </CyDTouchView>
                                  </CyDView>
                              </CyDView>}
                              {isEmailOTPVerified && <CyDView className={'flex flex-row justify-between items-center bg-paleDarkGreen mt-[10px] px-[8px] py-[10px] rounded-[5px]'}>
                                  <CyDText className={'text-[16px] pl-[8px] font-bold'}>
                                      {formData.email}
                                  </CyDText>
                                  <CyDImage source={AppImages.DARK_GREEN_BACKGROUND_TICK} />
                          ...
                          {isPhoneOTPVerified && isEmailOTPVerified && <CyDView>
                              <CyDTouchView onPress={() => { void submitVerification(); }}
                                  className={'bg-appColor py-[20px] flex flex-row justify-center items-center rounded-[12px] w-[100%] mx-auto mt-[25px]'}>
                                      <CyDText className={'text-center font-bold'}>{t<string>('PROCEED')}</CyDText>
                              </CyDTouchView>
                          </CyDView>}
                      </CyDView>
                  </CyDView>
