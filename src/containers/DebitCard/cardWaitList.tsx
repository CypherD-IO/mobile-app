...
export default function CardWailtList ({ navigation }) {
  ...
  const [selectedCountry, setSelectedCountry] = useState<{name?: string, dialCode?: string, flag?: string, Iso2?: string, Iso3?: string, currency?: string}>({ name: 'United States', dialCode: '+1', flag: '🇺🇸', Iso2: 'US', Iso3: 'USA', currency: 'USD' });
    ...
    if (userEmail && userEmail !== '') {
      if (isValidEmailID(userEmail)) {
        try {
          ...
          const payload = { email: userEmail, ethAddress: ethereum.address, country: selectedCountry.Iso2 };
          const response = await axios.post(`${ARCH_HOST}/v1/cards/waitlist`, payload);
          if (response.status === 201 && (response.data.status === 'REGISTERED' || response.data.status === 'PRESENT')) {
            setJoiningWaitlist(false);
            showModal('state', { type: 'success', title: '', description: t('JOIN_WAITLIST_SUCCESS_TOAST'), onSuccess: () => { setShowWaitlist(false); setUserEmail(''); hideModal(); }, onFailure: hideModal });
          }
        } catch (error) {
          Sentry.captureException(error);
          setJoiningWaitlist(false);
          showModal('state', { type: 'error', title: '', description: t('JOINING_WAITLIST_ERROR'), onSuccess: hideModal, onFailure: hideModal });
        }
      } else {
        setJoiningWaitlist(false);
      ...
      showModal('state', { type: 'error', title: '', description: t('EMPTY_EMAIL_ERROR'), onSuccess: hideModal, onFailure: hideModal });
    }
  }
    ...
    <CyDScrollView className='mt-[32px]'>
      ...
      <CyDView className={'w-screen'}>
        ...
        <CyDView>
          ...
          <CyDView className={'flex justify-center items-center text-center relative mt-[-130px]'}>
            <CyDView className={'w-[85%] bg-white px-[50px] pt-[100px] pb-[30px] rounded-[18px] shadow-lg'}>
              {!showWaitlist
                ? <>
                <CyDTouchView className={'mt-[5px] mb-[5px] border-[1px] border-inputBorderColor py-[12px] px-[10px] rounded-[5px] flex w-[100%]'} onPress={() => setModalVisible(true)}>
                  <CyDView className={clsx('flex flex-row justify-between items-center', { 'border-redOffColor': !selectedCountry })}>
                    <CyDView className={'flex flex-row items-center'}>
                ...
                <Button onPress={() => { onCountryUpdate(); }} loading={joiningWaitlist} style={'rounded-[8px] h-[50px] my-[15px]'} title={t<string>('CONTINUE_ALL_CAPS')}></Button>
              </>
                : <CyDView>
                <CyDTextInput value={userEmail} textContentType='emailAddress' autoFocus={true} keyboardType='email-address' autoCapitalize='none' autoCorrect={false} onChangeText={(text: string) => { setUserEmail(text); setIsValidUserEmail(true); }} placeholderTextColor={'#C5C5C5'} className={clsx('border-[1px] border-[#C5C5C5] h-[50px] rounded-[8px] text-center text-black', {
                  'pb-[10px]': isAndroid()
                })} placeholder='Enter your email'></CyDTextInput>
                <Button disabled={!isValidUserEmail && userEmail !== ''} onPress={async () => await joinWaitlist()} loading={joiningWaitlist} style={'rounded-[8px] h-[50px] mt-[20px]'} title={t<string>('CTA_JOIN_WAITLIST')}/>
                {!isValidUserEmail && userEmail !== '' && <CyDText className='text-center mt-[18px] mb-[-28px] text-red-500'>{t<string>('VALID_EMAIL_ERROR')}</CyDText>}
      ...
      <CyDView>
        <CyDText className={'mt-[25px] mx-[40px] text-[20px] font-bold'}>{t<string>('CARD_WAITLIST_PAGE_CAPTION')}</CyDText>
      </CyDView>
      ...
      <CyDView className={'flex items-center text-center'}>
        <CyDImage source={AppImages.DOTS_ILLUSTRATION} className={'w-[200px] h-[90px]'} />
      </CyDView>
    </CyDScrollView>
  );
}
