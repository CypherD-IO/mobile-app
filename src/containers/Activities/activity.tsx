...
function SentItem(props: any) {
  ...
  const formatDate = fromNow.includes('day')
    ...
    : fromNow;
  const formatAmount = `- ${DecimalHelper.toString(DecimalHelper.fromString(activity.amount), 6)} ${activity.tokenName}`;
...
function BridgeItem(props: any) {
  ...
  const formatDate = fromNow.includes('day')
    ? moment(activity.datetime).format('MMM DD, h:mm a')
    : fromNow;
  const formatFromAmount = `- ${DecimalHelper.toString(DecimalHelper.fromString(get(activity, ['fromTokenAmount'])), 4)} ${get(activity, 'fromSymbol')}`;
  const formatToAmount = `+ ${DecimalHelper.toString(DecimalHelper.fromString(get(activity, ['toTokenAmount'], '0')), 4)} ${activity.toSymbol}`;
...
function IBCItem(props: any) {
  ...
  const formatDate = fromNow.includes('day')
    ? moment(activity.datetime).format('MMM DD, h:mm a')
    : fromNow;
  const formatFromAmount = `- ${DecimalHelper.toString(DecimalHelper.fromString(activity.amount), 4)} ${activity.symbol}`;
  const formatToAmount = `+ ${DecimalHelper.toString(DecimalHelper.fromString(activity.amount), 4)} ${activity.symbol}`;
...
function WalletConnectItem(props: any) {
  ...
  const formatDate = fromNow.includes('day')
    ? moment(activity.datetime).format('MMM DD, h:mm a')
    : fromNow;
  const formatAmount = `- ${DecimalHelper.toString(DecimalHelper.fromString(activity.amount), 8)} ${activity.symbol}`;
...
function CardItem(props: any) {
  ...
  const formatDate = fromNow.includes('day')
    ? moment(activity.datetime).format('MMM DD, h:mm a')
    : fromNow;
  const formatAmount = `- ${DecimalHelper.toString(DecimalHelper.fromString(activity.amount), 6)} ${activity.tokenSymbol}`;
