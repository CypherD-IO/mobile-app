...
const CardCarousel = ({ setBannerHeight }: CardCarouselProps) => {
    ...
    const updateStatusForCardOrBridge = async (activity: BridgeOrCardActivity) => {
        ...
        const checkActivities = async () => {
            ...
            if (getPendingActivities().length === 0) {
                ...
                for (const recentActivity of recentActivities) {
                    const updatedActivity = await updateStatusForCardOrBridge(recentActivity);
    ...
    const renderItem = ({ item, index }: { item: BannerRecord | BridgeOrCardActivity, index: number }) => {
        return <CarouselItem
            item={item}
            index={index}
            boxWidth={boxWidth}
    ...
    const makeCards = () => {
        ...
        const highestPriorityCards = sortedBannerRecords
            .filter((staticCard) => staticCard.priority === 'HIGHEST');
