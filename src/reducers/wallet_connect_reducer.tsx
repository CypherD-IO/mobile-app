...
export const walletConnectInitialState: IWalletConnect = {
  connectors: [],
  dAppInfo: [],
  itemsAdded: false,
};
...
export const walletConnectReducer = (state, action) => {
  switch (action.type) {
    ...
    case WalletConnectActions.DELETE_DAPP_INFO:
      const key = state.connectors.findIndex(connector =>
        isEqual(connector, action.value.connector),
      );
    ...
    case WalletConnectActions.REMOVE_WALLETCONNECT_2_CONNECTION:
      if (state.dAppInfo?.length > 0) {
        const [connection] = state?.dAppInfo.filter(
          connectionObj => connectionObj?.sessionTopic === action.value.topic,
        );
        if (connection) {
          const key = state.connectors.findIndex(connector =>
            isEqual(connector, connection),
          );
          const nConnectors = state.connectors.filter((element, index) => {
            if (index !== key) {
              return element;
    ...
    case WalletConnectActions.RESTORE_SESSION:
      return {
        ...state,
        dAppInfo: action.value.dAppInfo,
        connectors: action.value.connectors,
        itemsAdded: true,
      };
